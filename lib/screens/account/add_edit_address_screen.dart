...
class AddEditAddressScreenState extends State<AddEditAddressScreen> {
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      _addressArg = ModalRoute.of(context)!.settings.arguments;
      if (_addressArg != null) {
        _addressArg = _addressArg as Map<String, String?>?;
      }

      if (_addressArg != null) {
        Provider.of<AddressList>(context, listen: false)
            .fetchAddressById(_addressArg['id'], context: context)
            .then((value) {
  ...
  Future<void> _saveForm() async {
    ...
    if (_editedAddress.id != null) {
      final response = await Provider.of<AddressList>(context, listen: false)
          .updateAddress(_editedAddress.id, _editedAddress, context);
      if (response == true) {
        status = 'Updated';
      }
    } else {
      try {
        final response = await Provider.of<AddressList>(context, listen: false)
            .addAddress(_editedAddress, context);
                          ...
                          child: SingleChildScrollView(
                            child: Column(
                              children: [
                                ...
                                // TextFormField(
                                //   initialValue: _initValues['countryId'],
                                //   decoration:
                                //       InputDecoration(labelText: TITLE_COUNTRY),
                                //   textInputAction: TextInputAction.next,
                                ...
                                //       cityId: _editedAddress.cityId,
                                //       pinCode: _editedAddress.pinCode,
                                //       defaultAddress:_editedAddress.defaultAddress,
                                //     );
                                //   },
                                // ),
